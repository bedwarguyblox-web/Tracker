...
import { Bell, BookOpen, Check, ChevronDown, CircleHelp, ClipboardList, Clock3, FilePenLine, Filter, History, Library, ListFilter, Menu, Moon, MoreHorizontal, Pencil, Pin, PinOff, Plus, Search, Settings, Sun, Trash2, UserRound, X } from 'lucide-react';
  ...
  return (
    <header className="site-header" data-testid="header-main">
      <div className="brand-lockup">
        <div className="brand-seal" aria-hidden="true"><Library size={19} strokeWidth={1.8} /></div>
        <div><div className="wordmark">MRC / <span>DEADLINE</span></div><div className="brand-caption">STUDENT PAPER TRAIL · 2024—25</div></div>
      </div>
      <div className="header-actions">
        ...
        <button className="icon-button mobile-menu" aria-label="Open menu" data-testid="button-mobile-menu"><Menu size={20} /></button>
      </div>
    </header>
  );
}
...
function DashboardStats({ active, urgent, done }: { active: number; urgent: number; done: number }) {
  return (
    <section className="stats-strip" aria-label="Deadline summary" data-testid="dashboard-stats">
      <div className="stat-card stat-primary"><div className="stat-label"><ClipboardList size={15} /> ON YOUR DESK</div><strong>{active}</strong><span>active deadlines</span></div>
      ...
      <div className="stat-card stat-done"><div className="stat-label"><Check size={15} /> FILED THIS TERM</div><strong>{done}</strong><span>completed items</span></div>
    </section>
  );
}
...
function TabsNav({ tab, onChange, historyCount }: { tab: Tab; onChange: (tab: Tab) => void; historyCount: number }) {
  return (
    <nav className="tabs-nav" aria-label="Deadline views" data-testid="tabs-nav">
      <button className={tab === 'active' ? 'tab active' : 'tab'} onClick={() => onChange('active')} data-testid="tab-active"><span className="tab-mark" />Current docket</button>
      <button className={tab === 'history' ? 'tab active' : 'tab'} onClick={() => onChange('history')} data-testid="tab-history"><History size={15} />Filed history <span className="count-pill">{historyCount}</span></button>
    </nav>
  );
}
...
function SearchFilterBar({ search, setSearch, subject, setSubject, priority, setPriority, subjects }: { search: string; setSearch: (v: string) => void; subject: string; setSubject: (v: string) => void; priority: string; setPriority: (v: string) => void; subjects: string[] }) {
  return (
    <div className="filter-bar" data-testid="search-filter-bar">
      <label className="search-field"><Search size={17} /><input type="search" placeholder="Search your docket…" value={search} onChange={(e) => setSearch(e.target.value)} data-testid="input-search" /></label>
      <div className="filter-controls"><label className="select-wrap"><ListFilter size={15} /><select value={subject} onChange={(e) => setSubject(e.target.value)} data-testid="select-subject"><option value="all">All subjects</option>{subjects.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="select-wrap"><Filter size={15} /><select value={priority} onChange={(e) => setPriority(e.target.value)} data-testid="select-priority"><option value="all">Any priority</option><option value="high">High priority</option><option value="medium">Medium priority</option><option value="low">Low priority</option></select></label></div>
    </div>
  );
}
...
function DeadlineCard({ item, onPin, onComplete, onEdit, onDelete }: { item: Deadline; onPin: () => void; onComplete: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <article className={`deadline-card ${status} ${item.pinned ? 'is-pinned' : ''}`} data-testid={`card-deadline-${item.id}`}>
      <div className="card-tab"><span>{item.subject}</span><button className={`pin-button ${item.pinned ? 'pinned' : ''}`} onClick={onPin} aria-label={item.pinned ? 'Unpin deadline' : 'Pin deadline'} data-testid={`button-pin-${item.id}`}>{item.pinned ? <Pin size={16} fill="currentColor" /> : <PinOff size={16} />}</button></div>
      <div className="card-body">
        <div className="card-topline"><span className={`urgency-stamp ${status}`}>{status === 'filed' ? 'Filed' : status === 'overdue' ? 'Overdue' : status === 'today' ? 'Today' : 'Upcoming'}</span><span className={`priority-dot ${item.priority}`} title={`${item.priority} priority`} /></div>
        <h3 data-testid={`text-deadline-title-${item.id}`}>{item.title}</h3>
        <p className="activity">{item.activity}</p>
        <div className="card-footer"><div className="due-block"><span className="due-label">{item.completed ? 'COMPLETED' : 'DUE DATE'}</span><strong>{item.completed ? readableDate(item.completedAt || item.dueDate) : readableDate(item.dueDate)}</strong><small>{item.completed ? 'Filed in history' : relativeDue(item.dueDate)}</small></div><div className="card-actions"><button className={`complete-button ${item.completed ? 'completed' : ''}`} onClick={onComplete} aria-label={item.completed ? 'Mark incomplete' : 'Mark complete'} data-testid={`button-complete-${item.id}`}><Check size={17} /></button><button className="more-button" aria-label="More actions" onClick={onEdit} data-testid={`button-edit-${item.id}`}><Pencil size={16} /></button><button className="delete-button" aria-label="Delete deadline" onClick={onDelete} data-testid={`button-delete-${item.id}`}><Trash2 size={15} /></button></div></div>
      </div>
    </article>
  );
}
...
function DeadlineList({ items, onPin, onComplete, onEdit, onDelete, tab }: { items: Deadline[]; onPin: (id: string) => void; onComplete: (id: string) => void; onEdit: (item: Deadline) => void; onDelete: (id: string) => void; tab: Tab }) {
  if (!items.length) return <div className="empty-state" data-testid="empty-deadlines"><div className="empty-icon"><Library size={25} /></div><h3>{tab === 'history' ? 'Nothing filed yet' : 'Clear desk, clear mind'}</h3><p>{tab === 'history' ? 'Completed work will appear here, in order.' : 'No deadlines match this view. Add one or loosen your filters.'}</p></div>;
...
function DeadlineModal({ initial, onClose, onSave }: { initial: Deadline | null; onClose: () => void; onSave: (data: Omit<Deadline, 'id' | 'createdAt' | 'completed' | 'completedAt'>) => void }) {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); if (title.trim() && activity.trim() && dueDate) onSave({ title: title.trim(), subject, activity: activity.trim(), dueDate, priority, pinned: initial?.pinned || false }); };
  return <div className="modal-backdrop" role="presentation" onMouseDown={(e) => { if (e.currentTarget === e.target) onClose(); }}><div className="deadline-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" data-testid="modal-deadline"><div className="modal-header"><div><span className="eyebrow">{initial ? 'EDIT RECORD' : 'NEW RECORD'}</span><h2 id="modal-title">{initial ? 'Amend deadline' : 'Add to the docket'}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close" data-testid="button-close-modal"><X size={19} /></button></div><form onSubmit={submit}><label>What is due?<input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Lab report conclusion" data-testid="input-title" /></label><div className="form-grid"><label>Subject<select value={subject} onChange={(e) => setSubject(e.target.value)} data-testid="input-subject"><option>Biology</option><option>English Literature</option><option>Mathematics</option><option>Modern History</option><option>Chemistry</option><option>French</option><option>Physics</option><option>Art & Design</option></select></label><label>Due date<input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} data-testid="input-due-date" /></label></div><label>Activity / notes<textarea value={activity} onChange={(e) => setActivity(e.target.value)} placeholder="A small note for future-you…" rows={3} data-testid="input-activity" /></label><fieldset><legend>Priority</legend><div className="priority-options">{(['low', 'medium', 'high'] as Priority[]).map((level) => <label key={level} className={`priority-choice ${level} ${priority === level ? 'selected' : ''}`}><input type="radio" name="priority" value={level} checked={priority === level} onChange={() => setPriority(level)} data-testid={`radio-priority-${level}`} /><span className="priority-dot" />{level}</label>)}</div></fieldset><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose} data-testid="button-cancel-modal">Cancel</button><button type="submit" className="button primary" data-testid="button-save-deadline"><Plus size={17} />{initial ? 'Save changes' : 'Add deadline'}</button></div></form></div></div>;
}

function ProfilePopover({ onClose }: { onClose: () => void }) {
  return <div className="profile-popover" data-testid="profile-popover"><div className="profile-popover-head"><span className="avatar large">AM</span><div><strong>Amina Malik</strong><span>Year 12 · MRC</span></div><button onClick={onClose} aria-label="Close profile"><X size={15} /></button></div><div className="profile-row"><Settings size={15} /> Demo profile <span className="demo-badge">LOCAL</span></div><div className="profile-row muted-row"><CircleHelp size={15} /> Shared with your cohort</div></div>;
}
...
function Home() {
  ...
  const saveDeadline = (data: Omit<Deadline, 'id' | 'createdAt' | 'completed' | 'completedAt'>) => {
    if (modal) { setDeadlines((all) => all.map((item) => item.id === modal.id ? { ...item, ...data } : item)); setToast('Record amended · saved locally'); }
    else { setDeadlines((all) => [{ ...data, id: `deadline-${Date.now()}`, createdAt: localDate(), completed: false }, ...all]); setToast('Added to your docket'); }
    setModal(false);
  };
  ...
  const deleteDeadline = (id: string) => { if (window.confirm('Remove this deadline from the shared docket?')) setDeadlines((all) => all.filter((item) => item.id !== id)); };
  return <div className="app-shell"><Header onAdd={() => setModal(null)} dark={dark} onTheme={() => setDark(!dark)} onProfile={() => setProfileOpen(!profileOpen)} />{profileOpen && <ProfilePopover onClose={() => setProfileOpen(false)} />}<main className="main-content"><div className="page-intro"><div><div className="eyebrow"><span className="eyebrow-rule" />MONDAY · {new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'long' }).format(today).toUpperCase()}</div><h1>Your academic <em>paper trail.</em></h1><p>A clear desk for the work ahead. Keep an eye on the urgent, then make room for what comes next.</p></div><button className="button primary add-button" onClick={() => setModal(null)} data-testid="button-add-deadline"><Plus size={18} /> Add deadline</button></div><DashboardStats active={active.length} urgent={urgent} done={history.length} /><TabsNav tab={tab} onChange={setTab} historyCount={history.length} /><SearchFilterBar search={search} setSearch={setSearch} subject={subject} setSubject={setSubject} priority={priority} setPriority={setPriority} subjects={subjects} /><div className="list-heading"><div><h2>{tab === 'active' ? 'Current docket' : 'Filed history'}</h2><span>{visible.length} {visible.length === 1 ? 'record' : 'records'} · shared with MRC students</span></div><span className="catalog-note"><span className="catalog-dot" /> Last synced just now</span></div><DeadlineList items={visible} onPin={(id) => setDeadlines((all) => all.map((item) => item.id === id ? { ...item, pinned: !item.pinned } : item))} onComplete={toggleComplete} onEdit={(item) => setModal(item)} onDelete={deleteDeadline} tab={tab} /><footer className="page-footer"><div><span className="footer-mark">MRC</span><span>Keep your paper trail honest.</span></div><span>Demo mode · changes stay on this device</span></footer></main>{modal !== false && <DeadlineModal initial={modal} onClose={() => setModal(false)} onSave={saveDeadline} />}{toast && <div className="toast-message" role="status" data-testid="status-toast"><span className="toast-check"><Check size={14} /></span>{toast}</div>}</div>;
}
